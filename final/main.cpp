//Nina Tseng and Alyssa Shen - Swagger Squirrel
...
// MOUSE FUNCTIONS AND RECTANGLE
    while(true){
        ...
        if(ev.mouse.x >= 90 && ev.mouse.y >= 150 && ev.mouse.x <= 200 && ev.mouse.y <= 260 && ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP){
            ...
            if(!shirtClicked){
                printImage (713, 413, 37); //hawaii shirt
        ...
        //when one item is clicked from each column and the total score is over 275, the user wins
        } else if (shirtClicked == true && pantsClicked == true && shoesClicked == true && accClicked == true && totalScore >= 275){
            //Exit when user enters 1 and continues if user enters 0.
            printEnd(again);
            if(again){
                reprintScreen(mainSquirrel.sAcc.value, mainSquirrel.sPants.value, mainSquirrel.sShirt.value, mainSquirrel.sShoes.value, shirtClicked, pantsClicked, shoesClicked, accClicked, highScore, totalScore);
            }else if(!again){
        ...
        //when one item is clicked from each column and the total score is under 275, the user loses
        } else if (shirtClicked == true && pantsClicked == true && shoesClicked == true && accClicked == true && totalScore < 275){
            al_rest(1);
            //print lose screen
            printImage (0,0,6);
